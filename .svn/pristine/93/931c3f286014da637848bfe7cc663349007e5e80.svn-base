		...
		if 'page' and 'count' in request.args:
			...
			try:
				...
				query2 = """ SELECT b.book_id, b.book_name, b.book_author,b.price, \
						...
						ON b.book_id = ba.book_id GROUP BY b.book_id;
						"""
				...
				total=len(books)
				total_pages = books1[0]['total_books']/count if len(books)>0 else 0
				total_books = books1[0]['total_books'] if len(books)>0 else 0
				
			except:
				logger.warn("there is some issue with the db")

			return make_response(jsonify({"status": "success", "response": books,"total": total2, "page": page,\
