...
class UserLogin(Resource):
	def post(self):
		...
		if role_id == 11:
			...
			cursor.execute(query,(role_id,username))
		else:
			query = """SELECT e.employeeId, e.employeeName, e.email, ur.role_name, 
		...
		if len(result)>0:
			...
			if cipher_front == cipher_db:
				...
				loginAt = now.strftime("%Y-%m-%d %H:%M:%S")
				query ="""INSERT INTO login_details(employeeId, loginAt, token) VALUES(%s, %s, %s);"""
				cursor.execute(query, (result[0]['employeeId'], loginAt, token))
				g.appdb.commit()
...
class UserLogout(Resource):
	def delete(self,employeeName):
		...
		if session.has_key(employeeName):
			...
			now = datetime.datetime.now()
			logoutAt = now.strftime("%Y-%m-%d %H:%M:%S")

			query = """UPDATE login_details SET logoutAt = %s, flag = 0 WHERE token = %s;"""
			cursor.execute(query,(logoutAt,token))
			g.appdb.commit()
