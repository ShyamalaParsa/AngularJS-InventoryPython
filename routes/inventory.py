...
class TotalInventoryDetails(Resource):
    ...
    def get(self):
        ...
        for device in devices:
            ...
            allotedquery = '''SELECT COUNT(ia.allocation_id) AS alloted FROM 
                            inventory_allocation ia, inventories i WHERE 
                            i.deviceId = %s AND i.inventory_id = ia.inventoryId 
                            AND ia.flag=1 AND i.flag=1'''
...
class AllotedDetails(Resource):
    ...
    def get(self):
        ...
        query = """SELECT emp.employeeName, emp.employeeId, dev.deviceName, inv.serial_number, inv.assetId 
                    FROM inventories inv, device_details dev, inventory_allocation inv_alloc, employee emp
                    WHERE inv.deviceId =dev.deviceId AND dev.deviceId=%s
                    AND inv_alloc.inventoryId=inv.inventory_id 
                    AND emp.employeeId=inv_alloc.employeeId
                    AND inv_alloc.flag = 1 AND inv.flag=1"""
